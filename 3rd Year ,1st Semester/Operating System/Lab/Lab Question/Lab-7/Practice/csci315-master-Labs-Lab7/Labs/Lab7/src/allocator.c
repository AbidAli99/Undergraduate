...
/*
	...
	void *size;
	*size = (void *) size;
	curNode = dlist_iter_begin(free_list); // start iterator at front
	while(curNode != NULL){
		if(curNode > size){ // if free_list node has enough space
			dlist_add_back(allocated_list, curNode, size);
			free_list->iter->size -= size; // decrease size of node in free_list
			free_list->iter->data += size; // move pointer in free_list
			return curNode;
		}
		curNode = dlist_iter_next(free_list); // set curNode data of next node
	}
	return NULL; // reaches end of free_list without allocating memory
*/
}
...
/*
	void *curNode;
	void *smallestNode;
	curNode = dlist_iter_begin(free_list);
	smallestNode = curNode; // set default smallest node to first node
	while(curNode != NULL){
		if(curNode > size && curNode < smallestNode){
			smallestNode = curNode;
		}
		curNode = dlist_iter_next(free_list);
	}
	if(smallestNode < size){ // if no node is large enough
		return NULL;
	}
	curNode = dlist_iter_begin(free_list);
	while(curNode != NULL){ // find smallest node: add space to free_list node
		if(curNode == smallestNode){
			dlist_add_back(allocated_list, smallestNode, size);
...
void *worstFit(size_t size){
	...
	while(curNode != NULL){
		if(curNode > size && curNode > largestNode){
			largestNode = curNode;
		}
		curNode = dlist_iter_next(free_list);
	}
	if(largestNode < size){ // if no node is large enough
		return NULL;
	}
	curNode = dlist_iter_begin(free_list);
	while(curNode != NULL){ // find largest node: add space to free_list node
		if(curNode == largestNode){
			dlist_add_back(allocated_list, largestNode, size);
			free_list->iter->size -= size;
			free_list->iter->data += size;
			return largestNode;
		}
