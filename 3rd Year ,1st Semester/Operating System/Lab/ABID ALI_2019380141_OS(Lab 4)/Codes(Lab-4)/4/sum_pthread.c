...
int main (int argc, char *argv[]) {
    ...
    struct timeval tv1;
	struct timeval tv2;
	struct timezone tz1; // just to have something to pass to gettimeofday()
	struct timezone tz2;
	gettimeofday(&tv1, &tz1);
	int startTime = tv1.tv_usec; // get time at start of program
    struct timeval tv;
    time_t t;
    struct tm *info;
    char buffer[64];
  ...
  t = tv.tv_sec;
	...
	int endTime = tv2.tv_usec; // get time at end of program
	printf("Total time elapsed in microseconds: %d\n", (endTime-startTime));
	info = localtime(&t);
